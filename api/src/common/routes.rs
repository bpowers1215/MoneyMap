...
pub fn get_routes() -> Router<ServerData> {
    ...
    router.delete("/money_maps/:mm_id/accounts/:acc_id", middleware! { |request, mut response|
        info!("API Endpoint: DELETE /money_maps/:mm_id/accounts/:acc_id");
        let sd: &ServerData = request.server_data();
        let mm_id = request.param("mm_id").unwrap().to_owned();
        let acc_id = request.param("acc_id").unwrap().to_owned();
        let result = &sd.controller_manager.accounts_controller.delete(request, mm_id, acc_id);
    ...
    // Account Statements
    router.get("/money_maps/:mm_id/accounts/:acc_id/statements", middleware! { |request, mut response|
        info!("API Endpoint: GET /money_maps/:mm_id/accounts");
        let sd: &ServerData = request.server_data();
        let mm_id = request.param("mm_id").unwrap().to_owned();
        let acc_id = request.param("acc_id").unwrap().to_owned();
        let result = &sd.controller_manager.account_statements_controller.find(request, mm_id, acc_id);
    ...
    router.get("/money_maps/:mm_id/accounts/:acc_id/statements/get_money_maps", middleware! { |request, mut response|
        info!("API Endpoint: GET /money_maps/:mm_id/accounts/:acc_id/statements/get_money_maps");
        let sd: &ServerData = request.server_data();
        let result = &sd.controller_manager.account_statements_controller.test_get_all_money_maps(request);
    ...
    router.get("/money_maps/:mm_id/accounts/:acc_id/transactions", middleware! { |request, mut response|
        ...
        let mm_id = request.param("mm_id").unwrap().to_owned();
        let acc_id = request.param("acc_id").unwrap().to_owned();
        let result = &sd.controller_manager.transactions_controller.find(request, mm_id, acc_id);

        response.set(MediaType::Json);
        JsonEncoder::encode(result)
    });
    router.post("/money_maps/:mm_id/accounts/:acc_id/transactions", middleware! { |request, mut response|
        info!("API Endpoint: POST /money_maps/:mm_id/accounts/:acc_id/transactions");
        let sd: &ServerData = request.server_data();
        let mm_id = request.param("mm_id").unwrap().to_owned();
        let acc_id = request.param("acc_id").unwrap().to_owned();
        let result = &sd.controller_manager.transactions_controller.create(request, mm_id, acc_id);
    ...
    router.delete("/money_maps/:mm_id/accounts/:acc_id/transactions/:tran_id", middleware! { |request, mut response|
        info!("API Endpoint: DELETE /money_maps/:mm_id/accounts/:acc_id");
        let sd: &ServerData = request.server_data();
        let mm_id = request.param("mm_id").unwrap().to_owned();
        let acc_id = request.param("acc_id").unwrap().to_owned();
        let tran_id = request.param("tran_id").unwrap().to_owned();
        let result = &sd.controller_manager.transactions_controller.delete(request, mm_id, acc_id, tran_id);
